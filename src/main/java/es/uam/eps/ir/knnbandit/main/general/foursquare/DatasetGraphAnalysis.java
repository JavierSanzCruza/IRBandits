...
public class DatasetGraphAnalysis
{
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        FastPreferenceData<Long, String> prefData = dataset.getPrefData();
        ...
        prefData.getAllUidx().forEach(u ->
        {
            ...
            long rel = prefData.getUidxPreferences(u).filter(i -> i.v2 >= threshold).mapToLong(i ->
            {
                prefData.getIidxPreferences(i.v1).filter(v -> !map.containsKey(v.v1) && v.v2 >= threshold).forEach(v -> uMap.addTo(v.v1, 1));
                return 1;
            }).sum();
            map.put(u, uMap);
            numRel.put(u, rel);
        ...
        {
            bw.write("num\tdensity\tnumComp\tavgWeight");
            for(int i = 0; i < densities.size(); ++i)
            {
                bw.write("\n" + i + "\t" + densities.get(i) + "\t" + numComp.get(i) + "\t" + avgWeight.get(i));
            }
        }
    }
}
