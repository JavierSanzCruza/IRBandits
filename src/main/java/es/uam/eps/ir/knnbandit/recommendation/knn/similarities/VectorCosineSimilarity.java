...
public class VectorCosineSimilarity implements UpdateableSimilarity
{
    ...
    public Stream<Tuple2id> similarElems(int idx)
    {
        try
        {
            ...
            {
                return this.num.get(idx).int2DoubleEntrySet().stream().filter(v -> v.getIntKey() != idx).map(v -> new Tuple2id(v.getIntKey(), v.getDoubleValue() / Math.sqrt(this.norm[idx]*this.norm[v.getIntKey()])));
            }
            return Stream.empty();
