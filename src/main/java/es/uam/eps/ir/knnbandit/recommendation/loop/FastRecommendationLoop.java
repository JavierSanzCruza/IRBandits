...
public interface FastRecommendationLoop<U,I> extends RecommendationLoop<U,I>
{
     ...
     * @return a triplet indicating: the selected user index, the item index if the algorithm is able to generate
     * a recommendation, and true if the user-item pair is used to update the metrics / system. If the algorithm
     * is not able to generate a recommendation, it returns null.
     */
    Tuple3<Integer, Integer, Boolean> fastNextIteration();
    /**
     ...
     *
     * @return true if (at least) the uidx-iidx pair is used for updating the recommendation algorithms, false otherwise.
     */
    boolean fastUpdate (int uidx, int iidx);
    /**
     * Updates the algorithms and metrics after receiving a metric
     * @param rec the recommendation ranking.
     */
    boolean fastUpdate(FastRecommendation rec);


    boolean fastUpdateNotRec (int uidx, int iidx);
    boolean fastUpdateNotRec(FastRecommendation rec);
    boolean fastUpdateRec(Stream<Pair<Integer>> pair);
    boolean fastUpdateRecList(Stream<FastRecommendation> recs);
    boolean fastUpdateRec(int uidx, int iidx);
    boolean fastUpdateRec(FastRecommendation rec);
