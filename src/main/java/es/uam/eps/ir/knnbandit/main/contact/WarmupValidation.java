...
public class WarmupValidation
{
    ...
    public static void main(String[] args) throws IOException, UnconfiguredException
    {
        ...
        if (warmupType == null)
        {
            System.err.println("ERROR: Invalid warmup type");
            return;
        }
        ...
        for (int part = 0; part < numParts; ++part)
        {
            ...
            ContactDataset<Long> validDataset = ContactDataset.load(dataset, partValid, notReciprocal);
            ...
            {
                case FULL:
                    warmup = new FullWarmup(validDataset.getPrefData(), partTrain, true, notReciprocal);
                    break;
                case ONLYRATINGS:
                default:
                    warmup = new OnlyRatingsWarmup(validDataset.getPrefData(), partTrain, true, notReciprocal);
            ...
            System.out.println("Relevant recommendations (with training): " + (validDataset.getNumRel(notReciprocal) - notRel));

            long a = System.currentTimeMillis();
            // Initialize the metrics to compute.
            Map<String, Supplier<CumulativeMetric<Long, Long>>> metrics = new HashMap<>();
            metrics.put("recall", () -> new CumulativeRecall<>(validDataset.getPrefData(), validDataset.getNumRel(notReciprocal), 0.5));
            ...
            algorithmSelector.configure(validDataset.getUserIndex(), validDataset.getItemIndex(), validDataset.getPrefData(), 0.5, notReciprocal);
            ...
            recs.entrySet().parallelStream().forEach((entry) ->
            {
                ...
                for (int i = 0; i < k; ++i)
                {
                    ...
                    RecommendationLoop<Long, Long> loop = new RecommendationLoop<>(validDataset.getUserIndex(), validDataset.getItemIndex(), validDataset.getPrefData(), rec, localMetrics, endcond, rngSeed, notReciprocal);
                    loop.init(warmup, true);
