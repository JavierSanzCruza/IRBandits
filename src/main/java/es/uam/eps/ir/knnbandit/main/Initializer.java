...
public class Initializer<U,I>
{
     ...
     */
    public Initializer(SimpleFastPreferenceData<U,I> validData, List<Tuple2<Integer,Integer>> training, boolean contactRec, boolean notReciprocal)
    {
        ...
        else IntStream.range(0, validData.numUsers()).forEach(uidx -> this.availability.add(new IntArrayList(itemList)));
        ...
        training.forEach(tuple ->
        {
            ...
            if(validData.numUsers(iidx) > 0 && validData.numItems(uidx) > 0 && validData.getPreference(uidx, iidx).isPresent())
            {
                ...
                if(notReciprocal)
                {
                    if(validData.numUsers(uidx) > 0 && validData.numItems(iidx) > 0 && validData.getPreference(iidx, uidx).isPresent())
                    {
                        this.cleanTraining.add(new Tuple2<>(iidx, uidx));
                    }
